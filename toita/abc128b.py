def main():
  ...
  def Ss_s(): return map(str,input().split())
  ...
  for i in range(n):
    s,p = Ss_s()
    restraunt_list.append([s,int(p),i+1])

  sd = sorted(restraunt_list,key=lambda x:x[1],reverse=True)

  sorted_data = sorted(sd,key=lambda x:x[0])
